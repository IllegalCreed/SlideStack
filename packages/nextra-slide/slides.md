...
| 路由模型 | App Router | 文件路由 | 文件路由 | 文件路由 | 配置文件 |
...
- 用户：SWR / Turborepo / Vercel AI SDK
...
- 自动 og-image、阅读时间、归档
- 适合：个人博客、公司技术博客
- 简洁但功能有限，复杂博客建议 Docs Theme + 手写列表
...
- 借助 `normalizePages()` 和 `getPageMap()` 拿到站点结构
- 借助 Tailwind / shadcn 任意自定义 UI
- 适合：品牌强诉求、文档 + 主站融合
...
export const metadata = {
  title: { default: 'My Docs', template: '%s — My Docs' },
  description: 'Nextra docs example',
}
...
const footer = <Footer>MIT {new Date().getFullYear()} © MyDocs.</Footer>
...
<Tabs items={['pnpm', 'npm', 'yarn']}>
  ...
  <Tabs.Tab>yarn add nextra nextra-theme-docs</Tabs.Tab>
</Tabs>
```
...
1. `next build` 完成后，`pagefind` 扫描产物 HTML
2. 生成 `_pagefind/` 目录（含索引 + 客户端 JS）
3. Nextra Theme 自动调用 `_pagefind/` 提供搜索 UI

**特点**

- 完全本地化 / 零成本
- 多语言分词内置
- 索引文件按需懒加载，1MB 站点典型索引 ~50KB
- 支持过滤代码块：`search: { codeblocks: false }` in `next.config.mjs`

</div>

<div v-click>

> 💡 **替代方案**：自部署的 [Algolia DocSearch](https://docsearch.algolia.com/) 免费给开源项目，跨语言搜索体验更好，但要走外部服务。
...
**Step 1：next.config.mjs 声明 locale**
...
/* app/globals.css */
:root {
  --x-color-primary-600: oklch(0.62 0.2 285); /* purple-500 */
}

html.dark {
  --x-color-primary-600: oklch(0.7 0.2 285);
}
```
...
const navbar = (
  <Navbar
    logo={<MyLogo />}
    projectLink="https://github.com/x/y"
...
**3. 完全 Swizzle（重，转 Custom Theme）**

迁移到 Custom Theme：从 `nextra/components` import 原子组件（`Anchor`、`Sidebar` 内部接口、`normalizePages` 等），自己拼装 Layout。
...
```js
nextra({
  defaultShowCopyCode: true,  // 全局加复制按钮
  // 也可在 mdxOptions.rehypePrettyCodeOptions 里自定义 Shiki theme
})
```
...
frontmatter 字段自动映射到 Next.js head 元素。
...
```tsx
// app/opengraph-image.tsx —— Next.js 15 标准
...
pnpm add next@latest react@latest react-dom@latest \
  nextra@latest nextra-theme-docs@latest

# 2. 把 pages/ 改名 content/，docs 文件搬过去
mv pages content
mv pages/_meta.ts content/_meta.ts

# 3. 删除 theme.config.jsx，把配置搬到 app/layout.tsx

# 4. 新建 app/[[...mdxPath]]/page.jsx + app/layout.tsx
```
...
> 💡 **守在 Pages Router 的用户**：Nextra 3 仍在维护安全更新，不强制升级；但新特性（如新的 Pagefind 集成、RSC 优化）只进 4.x。
...
# 或：GitHub repo 接进 Vercel Dashboard → 自动 PR 预览 + main 生产
```

Vercel 直接识别 Nextra（Next.js 默认配置），ISR / Edge / RSC 全开。
...
```bash
pnpm build              # 输出 out/
# 然后 Cloudflare Pages → 输出目录 out/，构建命令 pnpm build
```
...
- Nextra = Next.js 原生；VitePress = Vue + Vite，**完全不同的运行时**
- VitePress 构建快 10×，首屏小 3×（无 React 全家桶）
- Nextra 胜在「Next.js 团队复用现成栈」，VitePress 胜在「极致简洁 + 速度」
- **决策**：团队已用 React/Next.js → Nextra；团队中立或偏 Vue → VitePress
...
- Docusaurus = Meta 出品 React-based，**多版本 + 博客 + i18n** 是一等公民
- Nextra = Vercel 出品 Next.js-based，**Next.js 集成 + Pagefind** 更原生
- **决策**：要多版本 + 博客一体化 → Docusaurus；要 Next.js / Vercel 生态 → Nextra
...
- Starlight = Astro 的官方文档主题，Island 架构，**零 JS 首屏**
- Nextra 首屏 ~150KB（React + Next runtime），Starlight 接近 0
- **决策**：极致性能 / SEO 关键页 → Starlight；功能丰富 / 生态对接 → Nextra
...
- **MDX 3 严格性**：`{` 被识别为 JSX 表达式开始，`{ "key": "value" }` 这种 JSON 字面量会报错，用 `&#123;` 转义或代码块包裹
- **Turbopack 别名缺失**：dev 模式 MDX 组件不生效 → 检查 `next.config.mjs.turbopack.resolveAlias['next-mdx-import-source-file']`
- **缺失 `mdx-components.tsx`**：MDX 页面只有原生 HTML 无主题样式 → 必须创建这个文件
- **静态导出失去 middleware**：`output: 'export'` 下 i18n 自动检测不工作，要么换 Vercel 部署，要么手动切换
- **`suppressHydrationWarning` 漏掉**：darkmode 切换时 hydration mismatch 警告刷屏 → 根 html 标签加 `suppressHydrationWarning`
- **`_meta.ts` key 顺序**：sidebar 顺序由对象 key 顺序决定，文件名不影响
- **`content/` 路径**：Nextra 4 默认从 `content/` 读，从 Nextra 3 升级时容易忘改
- **Pagefind 索引路径**：postbuild 命令 `--site` 指向必须是 build 产物，常见误写 `.next` 而非 `.next/server/app`
- **App Router 限制**：Nextra 4 不支持 Pages Router，升级前先评估业务路由是否在 App Router 下
...
- [Built-In Components](https://nextra.site/docs/built-ins/callout) —— Callout / Cards / Steps / FileTree / Tabs 全集
- [Guide / Markdown](https://nextra.site/docs/guide/markdown) —— MDX 3 完整能力
...
- [Search / Pagefind](https://nextra.site/docs/guide/search) —— 搜索引擎集成
- [i18n](https://nextra.site/docs/guide/i18n) —— 多语言完整流程
- [Static Exports](https://nextra.site/docs/guide/static-exports) —— GitHub Pages / Cloudflare 部署
- [Custom Theme](https://nextra.site/docs/custom-theme) —— 完全自定义主题
- [Advanced / Twoslash](https://nextra.site/docs/advanced/twoslash) —— 类型提示型代码块
...
- [Showcase](https://nextra.site/showcase) —— SWR / Turborepo / Million.js 真实案例
- [GitHub Discussions](https://github.com/shuding/nextra/discussions) —— 官方答疑
- [Nextra Examples](https://github.com/shuding/nextra/tree/main/examples) —— 三大 theme 完整模板
