...
- ① 异步的 Cookie Store API 可在 Service Worker 读写 Cookie，但兼容性尚未拉齐
...
- **非法源**：`file:`、`data:` 下访问直接抛 **`SecurityError`**——本地双击 HTML 调试「localStorage 怎么炸了」多半是这条；用户禁站点数据同理
...
<!--
评论区、客服窗、播放器、SSO 都在此列。自查清单最后一条是关键：只有 Cookie 能靠 Storage Access API 要回来，其他存储没有解分区通道——下一页展开。
-->
...
- **只救 Cookie 型会话**：Firefox 明确只有 Cookie 可动态解除分区——localStorage/IndexedDB **没有解分区通道**
