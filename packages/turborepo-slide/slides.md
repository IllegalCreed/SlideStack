...
- task 定义
- 根 lockfile
- 根 `package.json` 的 `engines`
- `globalDependencies` / `globalEnv`
- 行为相关 flags（如 `--env-mode`）
...
- 支持通配符前缀 `MY_API_*`
- 框架前缀如 `NEXT_PUBLIC_*` / `VITE_*` 由 Framework Inference 自动包含
...
6. **`--scope` 完全移除**（v1.2 起 deprecated）：用 `--filter`
