...
> 💡 **对比**：Claude Code / Codex 无免费额度；**Gemini CLI：1000 次/天免费，含 1M 上下文 Gemini 3 Pro**。
...
| `gemini-2.5-pro` / `flash` / `flash-lite` | 1M | 稳定版（Pro / 平衡 / 最轻量） |
| `auto` | - | 自动路由（默认） |

```bash
gemini -m gemini-3-pro-preview     # 或交互里 /model 切
```

<v-click>

**默认 `auto`**：让 CLI 按任务自动选模型。Plan Mode 自动「规划 Pro / 实施 Flash」切换。
...
| 工具 | 用途 |
| --- | --- |
| `read_file` / `read_many_files` | 读单 / 多文件（`@` 触发，支持图片 / PDF） |
| `list_directory` | 列目录 |
| `write_file` / `replace` | 创建 / 覆盖 / 字符串替换（要审批） |
...
`@` 让 `read_many_files` 一次拉所有文件，git-aware 自动跳过 `node_modules/`、`.env`。
...
├── SKILL.md       # 主指令（必需，含 name + description）
├── references/    # 引用资源（示例 / 反例）
├── scripts/       # 工具脚本
└── data/          # 静态数据
```
...
# Cypress 测试规范
E2E 测试必须用 test 服务器（端口 10060，test DB）...
```
...
1. **Discovery**：CLI 启动扫所有 SKILL.md，`name + description` 注入 system prompt
2. **Activation**：模型匹配 description → 调 `activate_skill(name)` 工具
3. **Consent**：用户主动确认（一次性 / 永久）
...
**四级优先级**（低 → 高）：内置 → Extension 携带 → 用户（`~/.gemini/skills/`）→ Workspace（`.gemini/skills/`，commit 进仓库）。

</v-click>

<v-click>

> 💡 **Progressive Disclosure**：metadata 只塞 system prompt 几十 token，激活时才注入完整 SKILL.md——比 Claude Code 默认全注入更省 token。
...
| `BeforeAgent` / `AfterAgent` | Agent loop 开始 / 结束 |
| `BeforeModel` / `AfterModel` | LLM 调用前 / 返回后 |
| `BeforeToolSelection` | 工具选择前 |
| `BeforeTool` / `AfterTool` | tool 执行前 / 后 |
| `PreCompress` | 上下文压缩前 |
| `Notification` | 警告 / 通知 |
...
// ~/.gemini/settings.json
{
  "hooks": {
    "BeforeTool": [{
      "matcher": "write_.*",
      "hooks": [{ "name": "audit", "type": "command",
        "command": "echo \"$TOOL_NAME\" >> ~/audit.log", "timeout": 5000 }]
    }],
    "AfterTool": [{
      "matcher": "write_file|replace",
      "hooks": [{ "name": "prettier", "type": "command",
...
**stdout / stderr**：stdout 输出 JSON，stderr 打 debug 日志。

**退出码**：`0` 成功 / `0` + `{"decision":"deny"}` 拒绝 / `2` 系统级阻塞 / 其它 警告但放行。
...
> ⚠️ **Hook 是 shell 执行点**：Project 级 hook 改动会触发 fingerprint 警告，防恶意 hook 自动跑。
...
**常用字段**：`command` + `args`（stdio） / `url` / `httpUrl` / `headers`（远程鉴权） / `env` / `trust`（跳过审批） / `includeTools` / `excludeTools`（白黑名单）。
...
| `cloud-run` / `bigquery` / `firebase` | GCP 部署 / 查询 / 集成 |
...
```json
{ "agents": { "overrides": { "codebase_investigator": { "model": "gemini-3-pro-preview" } } } }
```
...
配置：`general.checkpointing.enabled = true`

<v-clicks>

启用后每次 `write_file` / `replace` 前自动：

1. **Shadow Git 提交**：`~/.gemini/history/<project_hash>` 仓库存当前状态
2. **会话快照**：`~/.gemini/tmp/<project_hash>/checkpoints/` 存对话历史 + tool 调用
...
```bash
/restore                                                  # 列出所有
/restore 2026-05-15T10-00-00_000Z-src-index.ts-write_file # 回滚
```
...
> 💡 **双重恢复**：`/restore` 不仅还原文件，对话也回到那时——Gemini 重新看到原 tool prompt。
...
export GEMINI_SANDBOX=docker    # 或环境变量指定
```

<v-click>

**自定义 sandbox 镜像**（`.gemini/sandbox.Dockerfile`）：
...
# CI 全自动
gemini -p "lint 检查并写报告" --approval-mode=yolo --output-format json
```

<v-click>

退出码：`0` 成功 / `1` 一般错误 / `42` 输入校验错 / `53` 超过 turn 限制。
...
自动装好 `.github/workflows/gemini-cli.yml`：PR 评审（`@gemini review`） / Issue 分诊 / 自定义命令触发 / 跨 PR 代码分析。
...
CI 内 `GEMINI_API_KEY` 走 GitHub Secrets，免费 1000 RPD 跑日常 PR 评审够用。
...
| --- | --- | --- |
| **开源 / 免费额度** | ✓ Apache 2.0 / **1000 RPD** | ✗ / 无 |
| **默认上下文** | **1M** | 200K |
| 模型 | Gemini 3 / 2.5 全系 | Opus / Sonnet / Haiku |
| 联网搜索 / Hooks | Google Search / **11 个** | WebFetch / 2 个 |
| Skills / Plan Mode / MCP | ✓ | ✓ |
| Checkpointing / Extensions / 沙箱 | ✓ | ✗ |
...
| **开源 / 免费额度** | ✓ Apache 2.0 / **1000 RPD** | ✗ / 无 |
| **上下文** | **1M** | 200K |
| 模型 / 联网 | Gemini 全系 / Google Search | GPT / o / WebSearch |
| Hooks / Skills / Plan Mode | ✓（11 事件） | ✗ |
| Checkpointing / Extensions | ✓ | ✗ |
| MCP | ✓ | ✓ |
...
| 回复被截断 / 高 token 用量 | `/compress` 或 `/stats` + 切 Flash |
| Hook 不触发 | `/hooks` 看 matcher 模式 |
...
- **GEMINI.md 是 prompt injection 面**：克隆陌生仓库先看 GEMINI.md
- **MCP / Extension 是代码执行点**：装前看源码，用 `security.allowedExtensions` 白名单
- **Hook 命令是 shell 注入面**：环境变量在 hook 里要 quote
- **token 别 commit**：`environmentVariableRedaction` 默认遮蔽 TOKEN / SECRET
- **Folder Trust**：首次进入新目录需用户确认信任
...
| 0.10 | 2025 末 | MCP / Plan Mode |
| 0.20 | 2026 初 | Skills / Checkpointing |
| 0.30 | 2026 初 | Hooks 11 事件 / Extensions |
| 0.42 | 2026 5 月 | 当前稳定版（Gemini 3 默认） |

<v-click>

**关键演进**：MCP + Plan Mode 早期对齐 Claude Code；Skills 按需加载更省 token；Checkpointing Shadow Git 独家；Extensions 一键装；11 hook 事件可编程性强。
...
**Week 1 — 上手**：装 + OAuth + 第一次对话；写 `GEMINI.md`；`/init` + 改小 bug。

**Week 2 — 进阶**：切换模型（Pro / Flash）；自定义 slash 命令（`.toml`）；Plan Mode + 自动路由。

**Week 3 — 扩展**：装 1-2 个 MCP server / Extension；写第一个 Hook；装社区 Skill。

**Week 4 — 工作流**：配 `~/.gemini/settings.json`；spawn subagent；开 checkpointing + 沙箱。
...
| GitHub / Releases | [google-gemini/gemini-cli](https://github.com/google-gemini/gemini-cli) |
...
| MCP / GitHub Action | [modelcontextprotocol.io](https://modelcontextprotocol.io/) / [run-gemini-cli](https://github.com/google-github-actions/run-gemini-cli) |
