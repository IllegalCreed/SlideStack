...
| 计划批准后 | `executing-plans` | mandatory |
| 大任务挤主线程 | `subagent-driven-development` | recommended |
| 多任务串行做完 | `dispatching-parallel-agents` | recommended |

<v-click>

`mandatory` = Agent **必须**走，跳过需用户显式同意；`recommended` = 优先用但可灵活判断；`optional` = 仅手动调用。
...
- `mandatory`：Agent **必须**走，跳过需用户显式同意
- `recommended`：优先用但可灵活判断
- `optional`：仅手动调用，不自动触发
...
**Skill** 影响 Agent **思考方式**，**Hook** 影响 Agent **工具调用**。

- Hook 强（不可绕过）
- Skill 灵活（Agent 看场景决定）
...
| 个性化 | 自定义 SKILL.md | 命令别名 + 配置 |

<v-click>

**选谁**：

- 想培养良好工程习惯 → Superpowers
- 想降低 Claude Code 使用门槛 → ECC
- 团队规范化 + 多人共享方法论 → Superpowers
- 个人轻度使用 → ECC 或不装
