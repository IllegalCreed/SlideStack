...
- **middleware 生态**：persist / devtools / immer / subscribeWithSelector
- **vanilla 可用**：脱离 React 也能跑（Web Worker / Vue / 原生 JS）
...
| 状态模型      | 单 store / immutable  | Slice / Immer       | 多 atom           | 单 proxy / mutable| 多 store          |
| TS 支持       | **curried 推导**      | 优                  | 优                | 优                | **原生推导**      |
| 包体积        | **~1 KB**             | ~10 KB              | ~3 KB             | ~3 KB             | ~1.5 KB           |
| DevTools      | Redux DevTools        | Redux DevTools      | Jotai DevTools    | Redux DevTools    | Vue DevTools      |
...
- Zustand = 「单一 store，hook 订阅」 → 适合大多数项目
- Jotai = 「多 atom，颗粒最小」 → 适合大量独立状态
- Valtio = 「proxy mutable」 → 适合习惯 Vue 思维的开发者
...
```tsx
// 每次 selector 调用都返回新对象 → 总是重渲
const { count, step } = useCounterStore((s) => ({
  count: s.count,
  step: s.step,
}));
```
...
| Middleware                 | 来自                                    | 作用                              |
| -------------------------- | --------------------------------------- | --------------------------------- |
| **persist**                | `zustand/middleware`                    | localStorage / sessionStorage 持久化 |
| **devtools**               | `zustand/middleware`                    | Redux DevTools 集成               |
| **immer**                  | `zustand/middleware/immer`              | mutable 写法（嵌套更新友好）      |
| **subscribeWithSelector**  | `zustand/middleware`                    | 启用 subscribe 接收 selector 函数 |
| **combine**                | `zustand/middleware`                    | 合并 state + 函数（自动类型推断） |
| **redux**                  | `zustand/middleware`                    | Redux reducer 风格适配            |
...
export const useThemeStore = create<ThemeState>()(
  persist(
    ...
    {
      name: "theme-storage", // localStorage key（必填）
  ...
  devtools(
    (set) => ({
      ...
      addItem: (item) => set(
        ...
        false,           // 第二参 replace：false 表示浅合并
        "cart/addItem",  // 第三参 action name（DevTools 显示用）
      ),
    }),
    {
      name: "CartStore",  // DevTools 实例名
...
- 时间旅行：点任何历史状态回到那一刻
- Action log：每次 set 都有名字 + payload 记录
- State 树：可视化当前 state，可手动 diff / commit / revert
- 跨 store：多个 devtools 实例同时显示
...
export const useUserStore = create<UserState>()(
  immer((set) => ({
    ...
    setCity: (city) => set((draft) => {
      draft.user.profile.address.city = city;  // 直接 mutable 赋值
    }),
  })),
);
```
...
immer 底层用 Proxy 拦截 draft 的修改，最后生成不可变的新对象。
...
**为什么需要 subscribeWithSelector？**

默认 `store.subscribe(listener)` 接收的是整个 state，
任何字段变化都会触发 listener。

`subscribeWithSelector` 让 subscribe 也支持 selector：
...
// 只在 paw 变化时触发
const unsub1 = useDogStore.subscribe((s) => s.paw, console.log);

// 接收 previous value
const unsub2 = useDogStore.subscribe(
  (s) => s.paw,
  (paw, prev) => console.log("changed from", prev, "to", paw),
);
...
**combine 写法（自动推断 State，只需写 actions）**
...
export const useStore = create(
  combine(
    { count: 0, name: "Zustand" },   // 初始 state（自动推断类型）
    (set) => ({                       // actions
      increment: () => set((s) => ({ count: s.count + 1 })),
    }),
  ),
...
适合小型 store / Demo / 不想写 type 的场景；大型项目仍推荐显式 `create&lt;State&gt;()`。
...
// 读
const current = positionStore.getState();
...
Next.js 服务端是「单进程多请求」—— 模块顶层的 store 会被所有用户共享，
用户 A 的状态会污染用户 B 的请求。必须每个请求新建一个 store。
...
TypeScript 不支持「部分泛型推断」——
如果 create 同时有泛型 `&lt;T&gt;` 和函数参数，TS 要么全推、要么全不推，
没法「我手动给 T，函数参数让你推」。
...
create<State>()((set, get) => ({
  // set / get 类型自动推断为 StateCreator<State, []>
  count: 0,
  increment: () => set((s) => ({ count: s.count + 1 })),
}));

// ❌ 这种写法 set/get 类型会变成 unknown
create<State>((set, get) => ({ /* ... */ }));
```
...
第一对 `()` 接收 State 类型 → 返回一个泛型已固定的函数 → 第二对接收 stateCreator。
...
// 测试 A 加了 item，测试 B 看到 A 加的 item → 测试不可靠
```
...
**Vitest 方案：`__mocks__/zustand.ts` 全局自动 reset**
...
const fishes = useBearStore((s) => s.fishes);
...
const addBear = useBearStore.use.addBear();
```
...
| 场景                            | 推荐方案              | 原因                                |
| ------------------------------- | --------------------- | ----------------------------------- |
| React + 全局 store（中小项目）  | **Zustand**           | 极简、无 Provider、TS 友好          |
| React + 大型团队 / 严格架构     | **Redux Toolkit**     | 规范化、社区成熟、招人简单          |
| React + 大量独立小状态          | **Jotai**             | atom 颗粒度小，按需订阅             |
| React + 习惯 Vue 思维           | **Valtio**            | proxy mutable，写起来像 reactive    |
| Vue 3 项目                      | **Pinia**             | 官方推荐                            |
| 异步数据 / 请求缓存             | **TanStack Query**    | Zustand 不是数据缓存层              |
| URL state（筛选 / 分页）        | URL params + nuqs     | Zustand 不该管路由状态              |
| 表单状态                        | React Hook Form       | Zustand 全局存表单 = 状态污染       |
| 单组件 / 父子                   | useState + Context    | Zustand 是 overkill                 |
...
**❌ 错误：destructure 后变成普通值**

```tsx
// state 是当前快照，destructure 出来的 count 不会更新
...
`useCounterStore()` 不传 selector 时返回整个 state 对象 ——
你 destructure 拿到的是「当前那一刻的值」，不是 React 引用。
更糟的是：不传 selector 等同于「订阅整个 state」，任何字段变化都重渲。
...
用 persist middleware 持久化 theme：

- 服务端渲染：`theme = 'light'`（initial state）
- 客户端 hydrate 前：`theme = 'light'`（matches）
- 客户端 hydrate 后：`theme = 'dark'`（从 localStorage 读）
- React 报警：`Hydration failed because the initial UI does not match...`

</v-click>

<v-click>

**解决方案 1：等 hydration 完成再渲染**
...
if (!hydrated) return null; // 或 placeholder
...
**解决方案 2：persist 的 skipHydration + 手动 rehydrate**
...
```ts
// bearSlice.ts
import { useFishStore } from "./fishStore";  // 引用了 fishStore
export const createBearSlice = (set) => ({
  eatFish: () => useFishStore.getState().removeFish(),
});

// fishStore.ts
import { useBearStore } from "./bearStore";  // 反过来又引用 bearStore
```
...
**正确做法 1：把相关 slice 合并到同一个 store**

```ts
// 用 slices pattern 合并到一个 useBoundStore
// bear / fish 共享同一个 set/get，互相调用不需要 import
```
...
**正确做法 2：抽 third-party「事件总线」**

```ts
// useEventBus.ts (Zustand 也可以做事件总线)
export const useEventBus = create(() => ({}));
useEventBus.subscribe(...) // 任何模块都能订阅
```
...
完整电商购物车，含 persist + immer + devtools
  ...
  devtools(
    persist(
      immer((set, get) => ({
        items: [],
        add: (item) => set((d) => {
          const exist = d.items.find((i) => i.id === item.id);
          exist ? exist.qty++ : d.items.push({ ...item, qty: 1 });
        }, false, "cart/add"),
        remove: (id) => set((d) => { d.items = d.items.filter((i) => i.id !== id); }, false, "cart/remove"),
        setQty: (id, qty) => set((d) => { const i = d.items.find(x => x.id === id); if (i) i.qty = qty; }, false, "cart/setQty"),
...
**2. 多字段用 useShallow 而非 Object.is**

```ts
// 多字段 selector 必须配 useShallow，否则总是新引用
...
```ts
// ❌ 把 total 存进 state 容易和 items 不同步
const useStore = create((set) => ({ items: [], total: 0, addItem: ... }));
// ✅ total 用 get() 即时计算
...
- 左侧时间线：每条记录显示 action name + payload
- 右侧 Diff：当前 state vs 上一刻 state（绿/红高亮）
- 滑动时间线：state 实时回滚到任意时刻
- 右上 Dispatcher：手动 dispatch 修改 state

</v-click>

<v-click>

**4. 多 store 共享 DevTools**

每个 devtools middleware 配不同 name → DevTools 下拉切换实例。
...
src/
  stores/
    use-auth-store.ts        # 单一职责：登录状态
    use-cart-store.ts        # 单一职责：购物车
    use-ui-store.ts          # 单一职责：UI 状态（侧栏开合、主题）
    use-bound-store.ts       # 合并多 slice 的大 store
    slices/
      bear-slice.ts          # bear 相关 state + actions
      fish-slice.ts          # fish 相关 state + actions
      shared-slice.ts        # 跨 slice action（如 addBoth）
    middleware/
      logger.ts              # 自定义 middleware
    selectors/
      cart-selectors.ts      # 派生选择器（如 itemCount）
  __mocks__/
    zustand.ts               # 测试自动 reset
```
...
- 文件名 kebab-case：`use-cart-store.ts`
- Hook 名 camelCase：`useCartStore`
- Store 内部字段 camelCase；action 用动词起头（`addItem` / `setQty`）
...
**2. persist 设置 version + migrate**

线上数据结构变更时需要迁移钩子，否则老用户报错。
...
token / 密码 / 信用卡号绝不能写入 localStorage。
...
Next.js / Remix 必须 vanilla createStore + Provider 模式。
...
`npx vite-bundle-visualizer` 或类似工具，确认 Zustand 占用 ~1 KB。
...
- [Zustand 官方文档](https://zustand.docs.pmnd.rs/) — 必读，结构清晰
- [Zustand GitHub](https://github.com/pmndrs/zustand) — 源码 + Issue + Discussions
...
- [zustand-persist-storage](https://www.npmjs.com/package/zustand-persist-storage) — IndexedDB / cookie 存储
- [zustand-querystring](https://www.npmjs.com/package/zustand-querystring) — URL 同步
- [zustand-computed](https://www.npmjs.com/package/zustand-computed) — computed 属性 middleware
...
- [Excalidraw](https://github.com/excalidraw/excalidraw) — 大型 Zustand 应用案例
- [React Three Fiber](https://github.com/pmndrs/react-three-fiber) — 同团队，深度集成 Zustand
- 《Micro State Management with React Hooks》— Daishi Kato 著
...
- 函数即 store —— create 一个函数，hook 一个订阅
- selector 必传 —— 字段级订阅是性能关键
- 多字段配 useShallow —— 避免新对象引用陷阱
- middleware 按需组合 —— persist / devtools / immer 洋葱模型
- SSR 需要 Provider —— store-per-request 避免污染

</div>

<div class="mt-12 flex justify-center gap-6">
  <a href="https://zustand.docs.pmnd.rs/" target="_blank" class="slidev-icon-btn">
    📖 官方文档
  </a>
  <a href="https://github.com/pmndrs/zustand" target="_blank" class="slidev-icon-btn">
