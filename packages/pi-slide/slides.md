...
- 极简哲学：4 工具 + < 1K token 系统提示，给模型留更多上下文
- 全透明：所有 tool call / 注入都可见
- 25+ provider：mid-session 切模型，DeepSeek / Ollama 大陆友好
- MIT 开源 + Tree-structured Sessions（JSONL 任意节点分叉）
- TS Extensions：完整自定义工具 + Hooks，hot reload

**缺点**

- 资料相对少（vs Claude Code）；学习曲线陡（自己写扩展）
- 无 MCP（作者主张 CLI + README 替代）
- 无 Plan Mode / Subagents / 内置 TODO（用文件 + bash spawn 解决）
- YOLO 默认：对新手不友好；0.x 版本仍可能 breaking change
...
| 别家做 | Pi 选择 |
| --- | --- |
| 20+ 内置工具 | 4 个：read / write / edit / bash |
| 10K+ token 系统提示 | < 1000 token |
| 内置 Plan / TODO Mode | PLAN.md / TODO.md 文件 |
| Sub-agents 内置 | bash spawn pi 自己 |
| MCP server | CLI + README |
| 权限询问 / 后台 bash | YOLO + 限工具集 / tmux |

</v-clicks>

<v-click>

> 💡 **不是反对功能，是反对「内置」**——需要时用文件 / bash / 扩展实现，主框架保持精简。
...
**Edit vs Write**：改局部用 `edit`（省 token）；建新文件 / 整文件重写用 `write`。

</v-click>

<v-click>

**Bash 替代专门工具**：`grep` / `find` / `ls` 都走 bash，不需单独 tool。
...
**三层 AGENTS.md**：用户全局 `~/.config/pi/AGENTS.md` / 项目级 `<project>/AGENTS.md`（commit） / 项目本地 `<project>/.pi/AGENTS.md`（不 commit）。冲突时**项目级 > 用户级**。

</v-click>

<v-click>

> 💡 **兼容 CLAUDE.md**——也会读 `CLAUDE.md`，但优先 `AGENTS.md`（跨工具通用）。
...
| `openai` / `google` | GPT-5 / Gemini 2.5 Pro | 推理 / 长上下文 1M |
| `deepseek` | V3 / Coder | 大陆友好 + 便宜 |
| `groq` / `cerebras` | Llama 70B 极快 | 最快推理 |
| `ollama` | 本地任何开源模型 | 完全本地 |
| `claude-pro` | OAuth Pro/Max | 走订阅 |

<v-click>

完整支持 25+ provider：xAI / OpenRouter / Cloudflare / Hugging Face / 通义 / Kimi / MiniMax / Mistral / Together / Fireworks 等。
...
| --- | --- | --- |
| TUI（默认） | `pi` | 日常交互 |
| Print / JSON | `pi -p "..."` / `pi --json -p "..."` | 单次执行 / CI |
...
const agent = new Agent({ model: "claude-sonnet-4-6", provider: "anthropic" });
...
export default {
  ...
  tools: {
    queryDB: {
      ...
      async execute({ sql }) {
        return JSON.stringify((await pgClient.query(sql)).rows);
      },
    },
  },
  hooks: {
...
| 工具 / 系统提示 | 4 / <1K token | 20+ / 10K+ token |
| MCP | ✗（CLI+README 替代） | ✓ 一类支持 |
| 权限 / 子代理 | YOLO / bash spawn | 四档 / Agent 工具 |
| Memory / 开源 | 自管文件 / MIT | 自动系统 / 闭源 |
| 大陆访问 | DeepSeek / Ollama | 需自备网络 |

<v-click>

**怎么选**：Anthropic 重度 + 稳定 → Claude Code；多 provider + 自定义 + 开源 → Pi；大陆开发者 → Pi（接 DeepSeek / Ollama）。
...
| 维度 | Pi | OpenCode |
| --- | --- | --- |
| 作者 | Mario Zechner | sst 团队 |
| 模型 / 工具 | 25+ / 4 个（极简） | 75+ / 完整工具集 |
| MCP / 子代理 | ✗ / bash spawn | ✓ / 内置 Plan/Build/explore |
| 主要语言 | TypeScript | TypeScript + Go |

<v-click>

**怎么选**：极简 + 控制力 → Pi；功能更全 + 开箱即用 → OpenCode。
...
| Framework | vs Pi |
| --- | --- |
| LangChain / LangGraph | LangChain 是「让你构建」；Pi 是「能直接跑」 |
| AutoGen（微软） | AutoGen 重「多 agent 编排」；Pi 主张「一个 agent + bash 就够」 |
| CrewAI | 适合「模拟一个公司」；Pi 适合「就是个程序员」 |
| Aider | Pi 的精神前辈；Pi 在扩展机制 / 多 provider 更完整 |
...
| --- | --- | --- |
| 谁做 / 部署 | Mario / Earendil；本地 / 自托管 | Anthropic；云托管 |
| 模型 / 协议 | 25+ provider；TS SDK | 仅 Anthropic；Messages API + beta |
| 计费 / 开源 | provider key 直付；MIT | 走 Anthropic 账单；闭源 |
...
| `/clear` / `/compact` | 清空会话 / 手动压缩历史 |
| `/usage` | 看 token 用量 |
| `/sessions` / `/resume <id>` | 看会话列表 / 恢复 |
| `/skills` / `/extensions` | 列已加载 skills / extensions |
| `/<custom>` | 用户/扩展自定义命令 |
...
| Flag | 说明 |
| --- | --- |
| `-p, --print <prompt>` / `--json` | Print 模式 / JSON 输出 |
| `--model` / `--provider` | 指定模型 / provider |
| `--tools <list>` | 限工具集（`read,bash`） |
| `--resume <id>` / `--from <msg-id>` | 恢复会话 / 从节点分叉 |
| `--debug` | 详细日志 |
| `--no-extensions` / `--no-skills` | 跳过扩展 / skills |
...
| 变量 | 作用 |
| --- | --- |
| `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` | Anthropic / OpenAI key |
| `GEMINI_API_KEY` / `DEEPSEEK_API_KEY` | Google / DeepSeek key |
| `GROQ_API_KEY` / `OLLAMA_BASE_URL` | Groq key / Ollama 地址 |
...
| 版本 | 时间 | 主要变化 |
| --- | --- | --- |
| 0.1 / 0.20 | 2025-09 / 11 | 内部使用 / 设计博客 |
| 0.40 / 0.50 | 2025-12 / 2026-01 | 扩展系统稳定 / OpenClaw 上线 |
| 0.60 | 2026-02 | RPC / SDK / JSON mode 完善 |
| 0.70 / 0.74 | 2026-04 / 05 | 加入 Earendil / 当前版本 |
...
**Week 1 — 上手**：装 + 配 provider（推荐 DeepSeek） / 第一次对话体会 4 工具透明性 / 写 `AGENTS.md`

**Week 2 — 多 Provider**：配 2-3 个 provider 试 mid-session 切换 / DeepSeek 规划 + Claude 实施 / 试 Ollama 本地

**Week 3 — 扩展**：装 community Skill（如 superpowers） / 写第一个 TS Extension / 完善 `auth.json`

**Week 4 — 嵌入**：JSON mode 接 CI / RPC mode 起 server / SDK 嵌入自家应用
...
| 资源 | 链接 |
| --- | --- |
| 官网 / GitHub | [pi.dev](https://pi.dev/) / [earendil-works/pi](https://github.com/earendil-works/pi) |
...
| Skills 标准 / 对比阅读 | [agentskills.io](https://agentskills.io/) / [Anthropic Managed Agents](https://platform.claude.com/docs/en/managed-agents/overview) |
